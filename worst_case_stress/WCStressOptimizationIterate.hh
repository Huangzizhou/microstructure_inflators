...
public:
    ...
    void writeMeshAndFields(const std::string &name) const {
        ...
        ScalarField<Real> j = m_objective.integrand();
        for (size_t p = 0; p < Base::m_params.size(); ++p) {
            ...
            ScalarField<Real> offsetDiff = offsetObj.integrand() - j;
            ...
            // Didn't we prove it doesn't change with perturbations of w??
            // Can test this with a separate experiment.
            // Is it because I'm using the displacement form?
            auto offsetC = PeriodicHomogenization::homogenizedElasticityTensorDisplacementForm(offset_w, *m_sim);
            offsetObj.setPointwiseWCS(
                    worstCaseFrobeniusStress(m_sim->mesh().element(0)->E(), offsetC.inverse(),
                        PeriodicHomogenization::macroStrainToMicroStrainTensors(offset_w, *m_sim)));
            offsetDiff = offsetObj.integrand() - j;
            offsetDiff *= 1.0 / delta;
            ...
            offsetC = m_objective.wcStress.Sh.inverse() + dC * delta;
            offsetObj.setPointwiseWCS(
                    worstCaseFrobeniusStress(m_sim->mesh().element(0)->E(), offsetC.inverse(),
                        PeriodicHomogenization::macroStrainToMicroStrainTensors(offset_w, *m_sim)));
            offsetDiff = offsetObj.integrand() - j;
...
protected:
    using Objective = IntegratedWorstCaseObjective<N, WCStressIntegrandTotal>;
