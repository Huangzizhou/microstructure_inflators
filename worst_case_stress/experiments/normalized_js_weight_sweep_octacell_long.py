import json, os
cmd = '$MICRO_DIR/worst_case_stress/WCSRemeshingOptimization_cli -p $MICRO_DIR/worst_case_stress/octacell_ex_2.msh -m $MICRO_DIR/materials/B9Creator.material $MICRO_DIR/worst_case_stress/at_target_job_octacell_ex_2.opt --JSWeight {jsWeight} --JVolWeight {jvolWeight} -d1 -P{P} -n 6000 --maxNormStep 1e-4 -v 0.00001 -O10 -o it_{name} -R'
workingDir = os.environ['SCRATCH'] + '/wcs_bdry_perturb/normalized_js_weight_sweep_octacell_long/{subdir}'
stdout = '{name}.txt'
