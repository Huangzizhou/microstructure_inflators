...
import hexlib
from hexlib import polygon_centroid, min_distance_point_line, simplex_vertices_to_whole_square, \
    simplex_to_whole_square

if len(sys.argv) != 6:
    print "usage: ./four-leaf-clover-creator.py <ab positioning> <bd positioning> <cb positioning> " \
          "<output wire> <output mesh>"
    print "example: ./four-leaf-clover-creator.py 0.8 0.6 0.7 output.wire output.msh"
    sys.exit(-1)
...
midpoint_left_vertices = simplex_vertices_to_whole_square([midpoint_left])
midpoint_right_vertices = simplex_vertices_to_whole_square([midpoint_right])
p1_middle_vertices = simplex_vertices_to_whole_square([p1_middle])
p3_middle_vertices = simplex_vertices_to_whole_square([p3_middle])
d_vertices = simplex_vertices_to_whole_square([d])

# create topology in parallelogram
vertices, edges = simplex_to_whole_square(vertices, edges)
