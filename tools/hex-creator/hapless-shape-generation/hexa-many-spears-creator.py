...
if len(sys.argv) != 6:
    print "usage: ./hexa-many-spears.py <size of triangles> <num of pillars> <thickeness of diamond pillars> " \
          "<output wire> <output mesh>"
    print "example: ./hexa-many-spears.py 1.0 3 0.3 output.wire output.msh"
    sys.exit(-1)
...
for index, top_point in enumerate(vertices_from_p1_to_p2):
    ...
    a1 = np.array([top_point[0] - pillars_thickness/2, top_point[1] - pillars_thickness/2])
    a2 = np.array([top_point[0] + pillars_thickness / 2, top_point[1] - pillars_thickness/2])

    b1 = np.array([top_point[0] - pillars_thickness/2, 0])
    b2 = np.array([top_point[0] + pillars_thickness/2, 0])

    triangle_top = [top_point, a1, a2]
    triangle_left = [a1, b1, b2]
    triangle_right = [a1, b2, a2]

    edges_descriptions = hexlib.polygon_to_edges_descriptions(triangle_top)
    ...
    pillar_triangles += hexlib.simplex_polygon_to_whole_parallelogram(triangle_top, parallelogram_side)
