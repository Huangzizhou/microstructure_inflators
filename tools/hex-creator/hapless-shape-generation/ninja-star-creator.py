...
def create_pillars_with_constant_spacing_and_thickness(line1, line2, num_pillars, thickness, gap):
    edges_descriptions = []
...
if len(sys.argv) != 8:
    print "usage: ./ninja-star-creator.py <size of triangles> <num of pillars> <relative size of pillar area (chirality)> <relative thickness (of pillars)> <ninja coefficient>" \
          "<output wire> <output mesh>"
    print "example: ./ninja-star-creator.py 0.8 3 0.6 0.9 0.8 output.wire output.msh"
    sys.exit(-1)
...
z_reflected = ab_unit * p1*p3*s*(1-p5) + w_reflected

# computing thickness and spacing
pillar_area = np.linalg.norm(z - q2)
thickness = thickness_ratio * (pillar_area / num_pillars)
spacing = (pillar_area - thickness) / (num_pillars - 1) - thickness


# CREATE VERTICES IN THE INITIAL (ORIGINAL) EQUILATERAL TRIANGLE
edges_descriptions, original_pillar_polygons = create_pillars_with_constant_spacing_and_thickness(np.array([q2, z]), np.array([z_reflected, q2_reflected]), num_pillars, thickness, spacing)
