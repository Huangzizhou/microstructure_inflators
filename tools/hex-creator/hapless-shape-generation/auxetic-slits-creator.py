...
def create_pillars_with_constant_spacing_and_thickness(line1, line2, num_pillars, min_thickness_ratio, max_thickness_ratio, joint_thickness_factor, joint_offset_factor, pillar_area):
    edges_descriptions = []
    ...
    pillar_direction = line2[0] - line1[0]
    unit_pillar_direction = pillar_direction / np.linalg.norm(pillar_direction)
    joint_offset = np.linalg.norm(pillar_direction) / 2 * joint_offset_factor

    edges_descriptions.append([next_point1, next_point2])

    for t in range(0, num_pillars):
        thickness = get_thickness(min_thickness_ratio, max_thickness_ratio, t+1, num_pillars)
        upper_thickness = thickness * joint_thickness_factor - 1e-5
        ...
        offseted_beginning_pillar1 = beginning_pillar1 + unit_pillar_direction * (joint_offset + 1e-5)
        offseted_end_pillar1 = end_pillar1 + unit_pillar_direction * (joint_offset + 1e-5)

        # now, depending on joint_thickness_factor, adjust beggining_pillar1 and end_pillar1
        new_beginning_pillar1 = midpoint_pillar1 - unit_vector * upper_thickness / 2
        new_end_pillar1 = midpoint_pillar1 + unit_vector * upper_thickness / 2

        if len(edges_descriptions) > 0:
            edges_descriptions[-1][1] = new_beginning_pillar1
        else:
            top_edge0 = [beginning_pillar1, new_beginning_pillar1]
        ...
        top_edge0 = []
        top_edge1 = [new_beginning_pillar1, new_end_pillar1]
        #top_edge3 = [new_end_pillar1, end_pillar1]
        #top_edge4 = [offseted_beginning_pillar1, new_beginning_pillar1]
        #top_edge5 = [offseted_end_pillar1, new_end_pillar1]

        pillar_edge = [midpoint_pillar1, midpoint_pillar2]
        resulting_pillar_edge = create_pillar_edge(pillar_edge)

        middle_pillar_point = resulting_pillar_edge[1]
        beginning_middle_pillar = resulting_pillar_edge[1] - thickness * unit_vector / 2
        end_middle_pillar = resulting_pillar_edge[1] + thickness * unit_vector / 2

        auxiliar_pillar_edge1 = [offseted_beginning_pillar1, beginning_middle_pillar]
        auxiliar_pillar_edge2 = [offseted_end_pillar1, end_middle_pillar]
        resulting_auxiliar_pillar_edge1 = create_pillar_edge(auxiliar_pillar_edge1)
        resulting_auxiliar_pillar_edge2 = create_pillar_edge(auxiliar_pillar_edge2)

        crossing_edge = [new_beginning_pillar1, resulting_auxiliar_pillar_edge2[1]]

        pillar_triangles.append([resulting_auxiliar_pillar_edge1[1]] + crossing_edge)
        pillar_triangles.append([new_end_pillar1] + crossing_edge)
        pillar_triangles.append([new_beginning_pillar1] + resulting_auxiliar_pillar_edge1)
        pillar_triangles.append([new_end_pillar1] + resulting_auxiliar_pillar_edge2)

        #pillar_edges = [top_edge0, top_edge1, top_edge2, top_edge3, top_edge4, top_edge5, resulting_auxiliar_pillar_edge1, resulting_auxiliar_pillar_edge2, resulting_pillar_edge]
        pillar_edges = [top_edge0, top_edge1]
        #pillar_edges = [top_edge1, top_edge2, resulting_pillar_edge]

        #pillar_polygons.append([midpoint_pillar1, beginning_pillar1, beginning_pillar2, resulting_pillar_edge[1], new_beginning_pillar1, end_pillar1])
    ...
    return edges_descriptions, pillar_polygons, pillar_triangles



if len(sys.argv) != 11:
    print "usage: ./auxetic-slits-creator.py <size of triangles> <num of pillars> <relative size of pillar area (chirality)> <min relative thickness (of pillars)> " \
          "<max relative thickness (of pillars)> <ninja coefficient> <joint thickness factor> <joint offset factor>" \
          "<output wire> <output mesh>"
    print "example: ./auxetic-slits-creator.py 0.8 3 0.6 0.9 0.8 0.9 output.wire output.msh"
    sys.exit(-1)
...
joint_thickness_factor = float(sys.argv[7])
joint_offset_factor = float(sys.argv[8])
out_wire = sys.argv[9]
out_mesh = sys.argv[10]
...
edges_descriptions, original_pillar_polygons, original_pillar_triangles = create_pillars_with_constant_spacing_and_thickness(np.array([q2, z]), np.array([z_reflected, q2_reflected]), num_pillars, min_thickness_ratio, max_thickness_ratio, joint_thickness_factor, joint_offset_factor, pillar_area)
edges_descriptions.append([z, q1])
pillar_polygons += original_pillar_polygons
pillar_triangles += original_pillar_triangles
...
for triangle in original_pillar_triangles:
    rotated_60 = rotate_at(b, 60, triangle, [], [])
    rotated_120 = rotate_at(b, 120, triangle, [], [])
    pillar_triangles += [rotated_60, rotated_120]
...
# same with pillar polygons
for triangle in original_pillar_triangles:
    translated_pillar_polygon = translate_at(b, center, triangle, [], [])
    rotated_60 = rotate_at(center, 60, translated_pillar_polygon, [], [])
    rotated_120 = rotate_at(center, 120, translated_pillar_polygon, [], [])
    rotated_180 = rotate_at(center, 180, translated_pillar_polygon, [], [])
    rotated_240 = rotate_at(center, 240, translated_pillar_polygon, [], [])
    rotated_300 = rotate_at(center, 300, translated_pillar_polygon, [], [])
    pillar_triangles += [translated_pillar_polygon, rotated_60, rotated_120, rotated_180, rotated_240, rotated_300]
...
for triangle in original_pillar_triangles:
    ...
    rotated_60 = rotate_at(center, 60, translated_pillar_polygon, [], [])
    rotated_300 = rotate_at(center, -60, translated_pillar_polygon, [], [])
    pillar_triangles += [translated_pillar_polygon, rotated_60, rotated_300]
...
for triangle in original_pillar_triangles:
    ...
    rotated_180 = rotate_at(center, 180, translated_pillar_polygon, [], [])
    rotated_240 = rotate_at(center, 240, translated_pillar_polygon, [], [])
    pillar_triangles += [rotated_120, rotated_180, rotated_240]
...
for triangle in original_pillar_triangles:
    ...
    rotated_240 = rotate_at(center, 240, translated_pillar_polygon, [], [])
    rotated_300 = rotate_at(center, 300, translated_pillar_polygon, [], [])
    pillar_triangles += [rotated_180, rotated_240, rotated_300]
...
for index, triangle in enumerate(pillar_triangles):
    resulting_triangle = transformation_matrix * np.array(triangle).transpose()
    triangle = np.asarray(resulting_triangle.transpose()) - 1.0
...
incenters_pillars_pairs = hexlib.add_polygons_incenters(pillar_triangles, vertices, edges)
...
for index, pillar_polygon in enumerate(pillar_polygons):
    ...
    pillar_node1 = resulting_pillar_polygon[0]
    pillar_node2 = resulting_pillar_polygon[3]
    new_beginning_pillar_node = resulting_pillar_polygon[4]
    beginning_node = resulting_pillar_polygon[1]
    end_node = resulting_pillar_polygon[5]
    joint_nodes_thickness = nodes_thickness * joint_thickness_factor

    pillar_direction = pillar_node2 - pillar_node1
    norm_pillar_direction = np.linalg.norm(pillar_node2 - pillar_node1)
    unit_pillar_direction = pillar_direction / norm_pillar_direction
    v1 = hexlib.find_vertex_in_list(vertices, resulting_pillar_polygon[0])

    if norm_pillar_direction < joint_nodes_thickness:
        new_vertex1 = pillar_node1 + unit_pillar_direction * (norm_pillar_direction - 1e-5);
    else:
        new_vertex1 = pillar_node1 + unit_pillar_direction * joint_nodes_thickness;

    pillar_node1 = new_vertex1
    vertices[v1] = new_vertex1

    pillar_nodes_custom_pairs.append([[pillar_node1], joint_nodes_thickness])
    pillar_nodes_custom_pairs.append([[pillar_node2], nodes_thickness])
...
thickness_void = spacing
ninja_void = p3*p1*s
min_resolution = 2 * max(max(3 / thickness_void, 3 / thickness), 3 / ninja_void)
...
hexlib.inflate_hexagonal_box_smarter(out_wire, 0.00001, 0.0, out_mesh, incenters_thickness_pairs + incenters_pillars_pairs + pillar_nodes_custom_pairs, chosen_resolution)
