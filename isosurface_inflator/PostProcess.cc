                 ...
                 const MeshingOptions      &opts)
{
    ...
    std::unique_ptr<SMesh> bcm;
    {
        ...
        while (!done) {
            ...
            if (meshedFullPeriodCell) {
                ...
                using Pt = PointND<N>;
                BBox<Pt> cellND(truncateFrom3D<Pt>(meshCell.minCorner), truncateFrom3D<Pt>(meshCell.maxCorner));
            ...
            if (remove_small_components(componentIndex, componentSize, vertices, elements)) {
                std::cout << "Removed " << bcm-> numVertices() - vertices.size() << " vertices"
                          ...
                          << " (small disconnected components)" << std::endl;
    ...
    if (snappedBB != meshCell) {
        std::runtime_error error("Bounding box error");
        std::cerr << "snappedBB: " << snappedBB << std::endl;
        std::cerr << "meshCell: " << meshCell << std::endl;
        if (N == 2) {
            std::cerr << "Generated mesh doesn't touch all meshing cell bounding box faces; the tiled microstructure would be disconnected." << std::endl
                      << "(Possibly the result of disconnected component removal.)"
                      << std::endl;
            error = std::runtime_error("Incomplete mesh.");
        }
        else { error = std::runtime_error("Snapping failed."); }
        std::cerr << "Dumping debug.msh" << std::endl;
        MeshIO::save("debug.msh", vertices, elements);
        throw error;
    }
