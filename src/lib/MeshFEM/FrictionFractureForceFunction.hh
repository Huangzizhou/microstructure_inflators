//
// Created by Davi Colli Tozoni on 11/06/18.
//

#ifndef FRICTIONFRACTUREFORCEFUNCTION_H
#define FRICTIONFRACTUREFORCEFUNCTION_H

#include "MSHFieldWriter.hh"
#include "FrictionContactForceFunction.hh"

template<typename Real, class _Mesh>
class FrictionFractureForceFunction : public NonLinearElasticityFunction<Real> {
public:
    typedef _Mesh    Mesh;
    ...
    FrictionFractureForceFunction(_Mesh &mesh, Real alpha, Real etaRelu, Real etaNorm, Real mu) : m_mesh(mesh), m_alpha(alpha), m_h(etaRelu), m_norm(etaNorm), m_mu(mu) { }

    virtual ~FrictionFractureForceFunction() = default;
    ...
    virtual std::vector<Real> evaluate(std::vector<Real> u) const override {
        ...
        MSHFieldWriter writer("frictionFractureForce.msh", m_mesh);
            ...
            for (size_t A = 0; A < ce.numNodes(); ++A) {
                ...
                for (size_t i = 0; i < N; i++) {
                    ...
                    value *= Quadrature<N-1, QUADRATURE_ORDER>::integrate([&](const EvalPt<N-1> &pt) {
                        ...
                        return phiInterpolant(pt) * m_h.value(uDiff.dot(normalNegative)) * NetaDerivative(i);
                    }, area);
                    ...
                    for (size_t k = 0; k < N; k++) {
                        for (size_t l = 0; l < N; l++) {
                            ...
                            value *= Quadrature<N - 1, QUADRATURE_ORDER>::integrate([&](const EvalPt<N - 1> &pt) {
                                ...
                                Real firstInnerPart = m_h.derivative(uDiff.dot(normalNegative)) * NetaDerivative(k) * normalNegative(l);
                                Real secondInnerPart = m_h.value(uDiff.dot(normalNegative)) * NetaSecondDerivative.row(k) * (identity.col(l) - normalNegative(l) * normalNegative);
...
#endif //FRICTIONFRACTUREFORCEFUNCTION_H
