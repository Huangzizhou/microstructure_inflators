...
# code.
use strict;
(scalar @ARGV != 3) && die("Usage: make_flipper.pl matopt_out.txt fields.msh problem_name\n");
...
my @fields = ("u", "u_neumann", "u_dirichletTargets", "E", "nu");
my $drawCalls = join("\\\n", map(qq(field="$_"; Call DrawField;), @fields));
`cat render.geo | sed 's/<LAST_ITER>/$lastIt/; s/<PREFIX>/$imagePrefix/; s/<DRAW_CALLS>/$drawCalls/' > tmp.geo`;
