...
class LUT:
    ...
    def filterYoung(self, EMin, EMax):
        EValues = self.youngPoissonAnisotropy[:, 0]
        outIndices = np.where(np.logical_or(EValues < EMin, EValues > EMax))
        self.removeIndices(outIndices)
...
def extract(dim, pat, directory, printableOnly = True, init = False):
    ...
    if not moduli or not aniso or not params:
        return []
