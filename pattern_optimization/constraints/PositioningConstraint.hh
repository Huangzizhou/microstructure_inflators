#ifndef POSITIONINGCONSTRAINT_HH
#define POSITIONINGCONSTRAINT_HH

namespace PatternOptimization {
    namespace Constraints {

        template<size_t N>
        struct PositioningConstraint : Constraint<N> {
            ...
            PositioningConstraint(const _Iterate &it)
                    : Base(ConstraintType::INEQUALITY)
            {
                if (!it.isParametric())
                    throw std::runtime_error("Positioning constraints only support parametric optimization.");
                // **Note**: nlopt enforces constraints of the form c_i <= 0, while we
                // defined our PositioningConstraint constraints in the form c_i >= 0.
                ...
                //if (m_C.rows() == 0) throw std::runtime_error("No positioning constraints found.");
            }
            ...
            virtual void writeDescription(std::ostream &os, const std::string &name) const override {
                if (m_C.rows() == 0)
                {
                    os << "No positioning constraints found!" << std::endl;
                }
                else{
                    // Positive values mean violation
                    os << "max Positioning violation: " << (m_C * m_paramVec).maxCoeff() << std::endl;
                    Base::writeDescription(os, name);
                }
            }

            virtual ~PositioningConstraint() { }
...
// Configuration to be applied by iterate factory
        template<class _Sim>
        struct IFConfigPositioningConstraint : public IFConfig {
            ...
            void configIterate(const std::unique_ptr<_Iterate> &it, ObjectiveTermNormalizations &/* normalizations */) const {
                ...
                auto pc = Future::make_unique<PositioningConstraint<N>>(*it);
                it->addConstraint("PositioningConstraint", std::move(pc));
            }
        };
...
#endif /* end of include guard: POSITIONINGCONSTRAINT_HH */
