...
void usage(int exitVal, const po::options_description &visible_opts) {
    cout << "Usage: PostProcessFEM_cli [options] FEM_mesh" << endl;
...
po::variables_map parseCmdLine(int argc, const char *argv[])
{
    ...
    visible_opts.add_options()("help", "Produce this help message")
        //("quad,q", 		 	         po::value<string>(),					 "quadMesh file in .obj format")
        ("outputData,d", 	         po::value<string>(),                    "output mesh")
        ("option,o",		         po::value<size_t>()->default_value(1),  "1: top displacement, 2: per quad center avg data")
        ;
	...
	if (vm.count("outputData") == 0) {
        cout << "Error: must specify output file" << endl;
        fail = true;
    }

	/* if (vm.count("quad") == 0) { */
        /* cout << "Error: must specify .obj quad file" << endl; */
        /* fail = true; */
    /* } */
...
// read quad mesh: TODO: do this later
...
/*             catch (...) { /1* Don't complain yet--try orthotropic *1/ } */
/*         } */
...
/* 		/1* cout << "---BEFORE---" << endl; *1/ */
/* 		/1* for (size_t i = 0; i < 5; ++i) *1/ */
/* 		/1* 	cout << i << "area is: " << totAreaPerQuad(i) << endl << avgStrainPerQuad(i) << endl; *1/ */
/* 		/1* cout << "---AFTER---" << endl; *1/ */
/* 		/1* for (size_t i = 0; i < 5; ++i) *1/ */
/* 		/1* 	cout << i << "area is: " << totAreaPerQuad(i) << endl << avgStrainPerQuad(i) << endl; *1/ */
...
int main(int argc, const char *argv[])
{
	...
	VField displ;
	MSHFieldParser<2> parser(args["mesh"].as<string>());
	displ = parser.vectorField("u");

	// go over each vertex and for the ones that are on the top store x, ux, uy
	vector<Real> x;
	vector<Real> y; // to double check the correctness
	vector<Real> ux;
	vector<Real> uy;
	Real eps = 0.0000000000000001;

	// compute the y-coordinate of the top 
	Real yMax = 0.0;
	for (size_t i = 0; i < inVertices.size(); ++i)
		if (inVertices[i][1] > yMax)
			yMax = inVertices[i][1];

	for (size_t i = 0; i < inVertices.size(); ++i)
	{
		Real distFromTop = std::abs(inVertices[i][1] - yMax);
		

		if (distFromTop > eps)
			continue;
		else {
			x.push_back(inVertices[i][0]);
			y.push_back(inVertices[i][1]);

			ux.push_back(displ(i)[0]);
			uy.push_back(displ(i)[1]);
		}
	}

	ofstream out;
	out.open(args["outputData"].as<string>());


	out << setprecision(16) << showpos;
	for (size_t i = 0; i < x.size(); ++i)
		out << x[i] << "\t" << ux[i] << "\t" << uy[i] << "\t" << y[i] << endl;
